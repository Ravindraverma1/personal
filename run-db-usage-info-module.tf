module "run-db-usage-info_module" {
  ...
  source                                = "./modules/run-db-usage-info"
  customer                              = var.customer
  env                                   = var.env
  aws_region                            = var.aws_region
  lambda_subnet_ids                     = [aws_subnet.app_b.id, aws_subnet.app_a.id]
  db_usage_lambda_rds_security_group_id = aws_security_group.db_usage_lambda_rds_security_group.id
